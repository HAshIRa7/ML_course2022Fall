...
def rbf(x_1, x_2, sigma=1.):
    ...
    '''
    distances = torch.exp((-1) / (2 * sigma**2) * ( -2 * x_1 @ x_2.T + torch.sum(x_1**2, dim=1)[:, None] + torch.sum(x_2**2, dim=1)[:, None].T))### YOUR CODE HERE
...
def hinge_loss(scores, labels):
    ...
    return torch.mean(torch.maximum(torch.zeros(len(scores.shape)), 1 - scores * labels))### YOUR CODE HERE
